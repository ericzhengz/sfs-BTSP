...
class BTSPMemory(nn.Module):
    ...
    def __init__(self, num_classes: int, num_bits: int, num_branches: int = 1,
        ...
        # 🔥 纯门控版本：添加T_eff buffer用于新门控策略（统一口径：T_eff = τ_e * ln(1/θ)）
        # 注意：T_eff 只注册一次，修改 theta/tau_e 后请调用 recompute_teff() 以保持一致
        tau_e_val = float(tau_e_steps)
        theta_val = max(float(theta), 1e-6)
        T_eff_val = tau_e_val * math.log(1.0 / theta_val)
    ...
    @torch.no_grad()
    def set_p_gate_for_branches(self, branch_ids, value: float) -> None:
        """
        Set p_gate for specific branches only; others unchanged.
        Args:
            branch_ids: Iterable[int] of branch indices
            value: float value to set for those branches
        """
        try:
            import torch as _torch
            if not hasattr(self, 'p_gate'):
                return
            vec = self.p_gate
            if not isinstance(branch_ids, (list, tuple)):
                branch_ids = [int(branch_ids)]
            val = float(value)
            for b in branch_ids:
                if 0 <= int(b) < int(vec.numel()):
                    vec[int(b)] = val
        except Exception:
            return
