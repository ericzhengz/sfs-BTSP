...
class DLineController:
    ...
    def epsilon_control_inversion(self, current_error: float, M: int, p_pre: float, T_eff: float) -> float:
        """
        ε-control 反演律：根据目标误差反推所需的p_gate
        
        理论关系: ε ≈ 1 - (1 - p_gate)^(M × p_pre × T_eff)
        反演: p_gate ≈ 1 - (1 - ε)^(1/(M × p_pre × T_eff))
        """
        if M <= 0 or p_pre <= 0 or T_eff <= 0:
            return 0.5  # 默认值
            
        # 目标遗忘率
        target_epsilon = self.epsilon_0
        
        # 有效交互次数
        effective_interactions = M * p_pre * T_eff
        
        if effective_interactions < 1e-6:
            return 0.5
        ...
        if target_epsilon >= 1.0:
            ...
            retention_rate = 1.0 - target_epsilon
            p_gate_target = 1.0 - math.pow(retention_rate, 1.0 / effective_interactions)
            
        # 限制在合理范围
        p_gate_target = max(0.001, min(0.999, p_gate_target))
        
        return p_gate_target
...
class EventDrivenControlSystem:
    ...
    def event_step(self, 
        ...
        if self.mode == ControlMode.SIDECAR_ANALYSIS:
            ...
            fusion_for_metrics = self.alpha * base_logits + (1 - self.alpha) * btsp_logits
        else:
            # B模式：可融合
            fused_logits = self.alpha * base_logits + (1 - self.alpha) * btsp_logits
