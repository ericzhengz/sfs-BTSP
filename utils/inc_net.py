...
def get_backbone(args, pretrained=False):
    ...
    # Offline / no-download quick test path
        ...
        class DummyBackbone(nn.Module):
            def __init__(self, dim=768):
                ...
                self.embed = nn.Sequential(
                    nn.Flatten(),
                    nn.Linear(3 * 224 * 224, dim),
                    nn.ReLU(inplace=True),
                )
            def forward(self, x):
                return self.embed(x)
        return DummyBackbone().eval()

    # SimpleCIL or SimpleCIL w/ Finetune
    if name == "pretrained_vit_b16_224" or name == "vit_base_patch16_224":
        model = timm.create_model("vit_base_patch16_224",pretrained=args.get("pretrained", True), num_classes=0)
        model.out_dim = 768
        return model.eval()
    elif name == "pretrained_vit_b16_224_in21k" or name == "vit_base_patch16_224_in21k":
        model = timm.create_model("vit_base_patch16_224_in21k",pretrained=args.get("pretrained", True), num_classes=0)
        model.out_dim = 768
        return model.eval()
    ...
    # SSF 
        ...
        if args["model_name"] == "aper_ssf"  or args["model_name"] == "ranpac" or args["model_name"] == "fecam":
            ...
            if name == "pretrained_vit_b16_224_ssf":
                model = timm.create_model("vit_base_patch16_224_ssf", pretrained=args.get("pretrained", True), num_classes=0)
                model.out_dim = 768
            elif name == "pretrained_vit_b16_224_in21k_ssf":
                model = timm.create_model("vit_base_patch16_224_in21k_ssf", pretrained=args.get("pretrained", True), num_classes=0)
                model.out_dim = 768
            return model.eval()
        else:
